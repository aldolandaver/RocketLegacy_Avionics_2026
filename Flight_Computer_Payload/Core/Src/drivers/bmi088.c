 ...
 ******************************************************************************
 * @file    bmi088.c
 * @brief   Implementación SPI del BMI088. Protocolo: bit7=1 para lectura,
 *          bit7=0 para escritura; en modo lectura el acelerómetro requiere
 *          un byte dummy adicional tras la dirección (datasheet 6.1).
 ******************************************************************************
 */
...
static uint8_t acc_read_reg(bmi088_handle_t *dev, uint8_t reg)
{
    ...
    /* El acelerómetro descarta el primer byte recibido (dummy) */
...
static bool acc_read_burst(bmi088_handle_t *dev, uint8_t reg, uint8_t *buf, uint8_t len)
{
    ...
    /* A partir de aquí el bus ya está "alineado": enviamos dummies y
     * recibimos los datos reales en buf. */
...
static uint8_t gyro_read_reg(bmi088_handle_t *dev, uint8_t reg)
{
    ...
    /* El giroscopio NO requiere byte dummy extra (a diferencia del accel) */
...
bool BMI088_Init(bmi088_handle_t *dev)
{
    cs_high(dev->acc_cs_port, dev->acc_cs_pin);
    cs_high(dev->gyro_cs_port, dev->gyro_cs_pin);
    HAL_Delay(1); /* t_startup del sensor tras power-on, datasheet 1 ms */

    /* El acelerómetro del BMI088 arranca en soft-power-down; una lectura
     * "dummy" de CHIP_ID es requerida por errata del fabricante antes de
     * que el registro responda de forma fiable. */
    ...
    /* Encender acelerómetro (modo activo) y esperar t_wake (~5 ms) */
    acc_write_reg(dev, BMI088_ACC_PWR_CONF_REG, 0x00u); /* PWR_CONF: activo, no suspendido */
    HAL_Delay(1);
    acc_write_reg(dev, BMI088_ACC_PWR_CTRL_REG, 0x04u); /* PWR_CTRL: acc_enable = 1        */
    HAL_Delay(5);

    /* ODR = 200 Hz, ancho de banda normal (osr4), rango ±24g para tolerar
     * los picos de aceleración del motor sin saturar el ADC. */
    acc_write_reg(dev, BMI088_ACC_CONF_REG, 0xACu);   /* bwp=normal, odr=200Hz */
    acc_write_reg(dev, BMI088_ACC_RANGE_REG, 0x03u);  /* ±24g */

    /* Giroscopio: rango ±2000 dps, ancho de banda 116 Hz / ODR 1000 Hz
     * internamente; se muestrea a la misma cadencia que el acelerómetro
     * desde la tarea (200 Hz) para simplificar la sincronización del KF. */
    ...
    gyro_write_reg(dev, BMI088_GYRO_LPM1_REG, 0x00u);       /* modo normal (no suspend) */
