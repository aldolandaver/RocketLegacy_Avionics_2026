 ...
 ******************************************************************************
 * @file    bmp390.c
 * @brief   Compensación de presión/temperatura del BMP390 en punto flotante.
 ******************************************************************************
 */
...
static void parse_calibration(bmp390_handle_t *dev, const uint8_t *raw)
{
    ...
    c->par_t1 = (float)nvm_t1 / 0.00390625f;
    c->par_t2 = (float)nvm_t2 / 1073741824.0f;
    c->par_t3 = (float)nvm_t3 / 281474976710656.0f;

    c->par_p1  = ((float)nvm_p1 - 16384.0f) / 1048576.0f;
    c->par_p2  = ((float)nvm_p2 - 16384.0f) / 536870912.0f;
    c->par_p3  = (float)nvm_p3 / 4294967296.0f;
    c->par_p4  = (float)nvm_p4 / 137438953472.0f;
    c->par_p5  = (float)nvm_p5 * 8.0f;
    c->par_p6  = (float)nvm_p6 / 64.0f;
    c->par_p7  = (float)nvm_p7 / 256.0f;
    c->par_p8  = (float)nvm_p8 / 32768.0f;
    c->par_p9  = (float)nvm_p9 / 281474976710656.0f;
    c->par_p10 = (float)nvm_p10 / 281474976710656.0f;
    c->par_p11 = (float)nvm_p11 / 36893488147419103232.0f;
}
...
bool BMP390_Init(bmp390_handle_t *dev)
{
    ...
    if (!write_reg(dev, BMP390_REG_ODR, 0x02u)) return false;   /* ODR ~50 Hz */
...
float BMP390_PressureToAltitude(float pressure_pa, float ref_pressure_pa)
{
    ...
    const float exp_term = 0.1902225604f;
